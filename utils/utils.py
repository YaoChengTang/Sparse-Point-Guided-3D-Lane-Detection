...
def define_args():
    ...
    parser.add_argument('--balance', action='store_true', default=True, help='point sampling in balance mode')
    parser.add_argument('--near_sampling', action='store_true', default=True, help='sampling bg point near lane')
    parser.add_argument('--neg_num', type=int, default=1000, help='negative points number in point sampling')
    parser.add_argument('--pos_num', type=int, default=1000, help='positive points number in point sampling')
    parser.add_argument('--norm_point', action='store_true', default=False, help='normalize the sampled point coordinates into 0~1 during point sampling')
    parser.add_argument('--freeze', action='store_true', default=False, help='freeze baseline network')
    # Visualization setting
    parser.add_argument('--vis', action='store_true', help='only perform visualization')
    parser.add_argument('--vis_seg_id', type=str, default="9907794657177651763_1126_570_1146_570", help='model to train')
    # PersFormer settings
    parser.add_argument('--use_all_fea', action='store_true', default=False, help='use_all_fea')
    parser.add_argument('--model_name', type=str, default='PersFormer', help='model name')
    ...
    # implicit lane
    parser.add_argument('--lane2d_seg_num_block', type=int, default=3, help='implicit block number for lane2d segmentation')
    parser.add_argument('--lane2d_offset_num_block', type=int, default=3, help='implicit block number for lane2d offset prediction')
    parser.add_argument('--lane3d_seg_num_block', type=int, default=3, help='implicit block number for lane3d segmentation')
    parser.add_argument('--lane3d_offset_num_block', type=int, default=3, help='implicit block number for lane3d offset prediction')
    # Cluster
    parser.add_argument('--num_samples_list', type=list, default=[300,300,300,300], help='sampling number for each lane category in clustering')
    # LaneATT settings
...
class Visualizer:
    def __init__(self, args):
        self.save_path = ops.join(args.save_path, args.data_dir.split("/")[-2])
        print("saving vis to {}".format(self.save_path), args.save_path, args.data_dir.split("/")[-2])
    ...
    def draw_nothing_on_img_category(self, img, pred_anchors, gt_anchors, P_g2im, draw_type='laneline'):
        """
        :param img: image in numpy array, each pixel in [0, 1] range
        :param lane_anchor: lane anchor in N X C numpy ndarray, dimension in agree with dataloader
        :param P_g2im: projection from ground 3D coordinates to image 2D coordinates
    ...
    def draw_3d_curves_category_pred(self, pred_anchors, gt_anchors, h_cam, draw_type='laneline'):
        fig = plt.figure(dpi=100, figsize=(12,6))
        plt.style.use('seaborn-white')
        plt.rc('font', family='Times New Roman', size=10)
        ax = fig.gca(projection='3d')
    ...
    def save_result_new(self, dataset, train_or_val, epoch, idx, images, gt, pred, pred_cam_pitch, pred_cam_height,
        ...
        for i in range(idx.shape[0]):
            ...
            fig_img_2d = self.draw_nothing_on_img_category(im_laneline, pred_anchors, gt_anchors, P_gt, 'laneline')
            fig_img_ipm = self.draw_on_ipm_category(ipm_laneline, pred_anchors, gt_anchors, 'laneline')
            fig_pred = self.draw_3d_curves_category_pred(pred_anchors, gt_anchors, extrinsics[i][2,3], 'laneline')
            ...
            if evaluate:
                ...
                filename = self.save_path + '/vis_2d_img/' + '{}'.format(img_name[i])
                mkdir_if_missing(os.path.dirname(filename))
                fig_img_2d.savefig(filename,dpi=300)

                filename = self.save_path + '/vis_ipm_img/' + '{}'.format(img_name[i])
                mkdir_if_missing(os.path.dirname(filename))
                fig_img_ipm.savefig(filename,dpi=300)

                filename = self.save_path + '/vis_3d_pred/' + '{}'.format(img_name[i])
                mkdir_if_missing(os.path.dirname(filename))
                fig_pred.savefig(filename, bbox_inches='tight',dpi=300)
