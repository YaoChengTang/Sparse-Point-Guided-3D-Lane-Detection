...
class PersFormer(nn.Module):
    def __init__(self, args):
        ...
        self.use_all_fea = args.use_all_fea
        print("!"*30, "using all features" if self.use_all_fea else "using coarse results")
    ...
    def forward(self, input, _M_inv = None):
        ...
        info = "frontview_features: "
        for layer_idx, ele in enumerate(frontview_features) :
            info += "{}: {}, ".format(layer_idx, ele.shape)
        # print("-"*30, info)

        laneatt_proposals_list = self.laneatt_head(frontview_final_feat)
        info = "laneatt_proposals_list: {}".format(len(laneatt_proposals_list))
        for idx, laneatt_proposals in enumerate(laneatt_proposals_list) :
            info += "\r\n {}: ".format(idx)
            for ele in laneatt_proposals :
                info += " {}, ".format(ele.shape)
        # print("-"*30, info)
        ...
        # info = "projs: "
        # for layer_idx, ele in enumerate(projs) :
        #     info += "{}: {}, ".format(layer_idx, ele.shape)
        # print("-"*30, info)
        ...
        info = "bev_feat: {}".format(bev_feat.shape)
        # print("-"*30, info)

        out = self.lane_out(bev_feat)
        
        info = "out: {}".format(out.shape)
        # print("-"*30, info)
        ...
        info = "pred_seg_bev_map: {}".format(pred_seg_bev_map.shape)
        # print("-"*30, info)
