...
class Laneline_loss_gflat_multiclass(nn.Module):
    ...
    def forward(self, pred_3D_lanes, gt_3D_lanes, pred_hcam=None, gt_hcam=None, pred_pitch=None, gt_pitch=None, idx=None):
        """
        ...
        #     torch.log(torch.ones_like(pred_visibility) - pred_visibility + torch.tensor(1e-9))) / self.num_y_steps
        # assert torch.isnan(loss0).sum()==0, "In {}, loss0 has {} nan, pred_visibility: {}/{}".format( idx, torch.isnan(loss0).sum(), pred_visibility.max(), pred_visibility.min() )
        # turn for combination of sigmoid and binary cross-entropy
        max_val = (-pred_visibility).clamp(min=0)
        loss0 = torch.sum(
                    valid_category_weight * ( pred_visibility - pred_visibility*gt_visibility + max_val + 
                                              ((-max_val).exp() + (-pred_visibility-max_val).exp()).log() )
                ) / self.num_y_steps
        tmp = ((-max_val).exp() + (-pred_visibility-max_val).exp()).log()
        assert torch.isnan(loss0).sum()==0, "In {}, loss0 has {} nan, pred_visibility: {}/{}, tmp: {}/{}".format( idx, torch.isnan(loss0).sum(), pred_visibility.max(), pred_visibility.min(), tmp.min(), tmp.max() )
        ...
        assert torch.isnan(loss1).sum()==0, "In {}, loss1 has {} nan, pred_visibility: {}/{}".format(idx, torch.isnan(loss1).sum(), pred_category.max(), pred_category.min())
        ...
        if self.no_3d:
            ...
            loss2 = torch.sum(torch.norm(valid_category_weight*torch.cat((gt_visibility, gt_visibility), 3) *
                                     (pred_anchors-gt_anchors), p=1, dim=3))
        assert torch.isnan(loss2).sum()==0, "In {}, loss2 has {} nan, pred_visibility: {}/{}".format(idx, torch.isnan(loss2).sum(), pred_anchors.max(), pred_anchors.min())
