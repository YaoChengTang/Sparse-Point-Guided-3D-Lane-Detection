...
from models.ImplicitLane import ImplicitPersFormer
...
from models.networks.loss import implicit_loss_func
...
def generate_video(img_dir, sv_dir, video_name="try.mp4"):
    sv_path = os.path.join(sv_dir, video_name)
    print("We will save the generated video at {}".format(sv_path))

    images = [img for img in os.listdir(img_dir)
              if img.endswith(".jpg") or
                 img.endswith(".jpeg") or
                 img.endswith("png")]
    images.sort(key=cmp_to_key(lambda x,y: int(x.split(".")[0]) - int(y.split(".")[0])))
    print("There are {} images in {}".format(len(images), img_dir))
    ...
    video  = cv2.VideoWriter(sv_path, fourcc, 1, (width, height)) 
...
class Runner:
    ...
    def train(self):
        ...
        elif args.model_name.lower() == "implicitpersformer" :
            model, optimizer, scheduler, best_epoch, lowest_loss, best_f1_epoch, best_val_f1 = self._get_model_ddp(train_dataset)
        ...
        for epoch in range(args.start_epoch, args.nepochs):
            ...
            # Start training loop
            # for i, (json_files, input, seg_maps, gt, gt_laneline_img, idx, gt_hcam, gt_pitch, gt_intrinsic, gt_extrinsic, aug_mat, seg_name, seg_bev_map,   inst_seg_bev_map, inst_seg_map,   points_center_bev_map, points_center_2d_map,   points_coord_bev, points_coord_seg_bev,   points_coord_2d, points_coord_seg_2d) in tqdm(enumerate(train_loader)):
            for i, (json_files, input, seg_maps, gt, gt_laneline_img, idx, gt_hcam, gt_pitch, gt_intrinsic, gt_extrinsic, aug_mat, seg_name, seg_bev_map, M, H_g2im, P_g2im, H_crop, H_im2ipm) in tqdm(enumerate(train_loader)):
                # if M is None or len(M)==0:
                #     print("*"*60)
                #     print(json_files)
                #     print(input.shape, seg_maps.shape, torch.unique(seg_maps))
                #     print(gt.shape, gt_laneline_img.shape)
                #     print(idx,  gt_hcam, gt_pitch, gt_intrinsic.shape, gt_extrinsic.shape, aug_mat.shape)
                #     print(seg_name)
                #     print(seg_bev_map.shape, torch.unique(seg_bev_map))
                #     # print("-    -"*10)
                #     # print(inst_seg_bev_map.shape, inst_seg_map.shape, torch.unique(inst_seg_bev_map), torch.unique(inst_seg_map))
                #     # print(points_center_bev_map.shape, points_center_2d_map.shape)
                #     # print(points_coord_bev.shape, points_coord_seg_bev.shape, torch.unique(points_coord_seg_bev))
                #     # print(points_coord_2d.shape, points_coord_seg_2d.shape, torch.unique(points_coord_seg_2d))
                #     print("*"*60)
                ...
                if args.model_name.lower() == "persformer" :
                    # Inference model
                    ...
                    lane2d_result, lane3d_result_list, pred_hcam, pred_pitch, pred_seg_bev_map, uncertainty_loss = model(input=input, _M_inv=M_inv, aug_mat=aug_mat, 
                                                H_g2im=H_g2im, P_g2im=P_g2im, H_crop=H_crop, H_im2ipm=H_im2ipm,
                                                intrinsic=gt_intrinsic, extrinsic=gt_extrinsic)
                    
                    # loss
                    anchor_y_steps = train_dataset.anchor_y_steps
                    anchor_x_steps = train_dataset.anchor_x_steps if train_dataset.use_default_anchor else train_dataset.anchor_grid_x   #(N_anchor, N_y)
                    loss = implicit_loss_func( gt_pitch, pred_pitch, 
                                               gt_hcam, pred_hcam, 
                                               seg_bev_map, pred_seg_bev_map, 
                                               gt_laneline_img, lane2d_result, 
                                               gt, lane3d_result_list, 
                                               args=args, 
                                               loss_line2d_func=model.module.laneatt_head.loss, 
                                               loss_line3d_func=self.criterion, 
                                               N_y=self.args.num_y_steps, 
                                               anchor_x_steps=anchor_x_steps, 
                                               step=np.sqrt( np.power(self.args.sampling_step,2).sum() ),
                                            )
                    loss_seg, \
                    loss_att, loss_att_dict, \
                    loss_3d, loss_3d_dict = loss

                    # overall loss
                    loss = self.compute_loss(args, epoch, loss_3d, loss_att, loss_seg, uncertainty_loss, loss_3d_dict, loss_att_dict)
                
                elif args.model_name.lower() == "genlanenet":
                ...
                if (i + 1) % args.print_freq == 0 and args.proc_id == 0:
                    if args.model_name.lower() == "persformer" :
                        ...
                        print('Epoch: [{}][{}/{}]\t'
                            'Time {:.3f} ({:.3f})\t'
                            'Loss: {:.3f}, loss_seg: {}, loss_att: {}, loss_3d: {}, \t'
                            'uncertainty_loss: {}'.format(epoch+1, i+1, len(train_loader), 
                                                        batch_time.val, data_time.avg, 
                                                        loss, loss_seg, loss_att, loss_3d,
                                                        uncertainty_loss.detach().cpu().numpy()[:3]))
    ...
    def validate(self, model, model2=None, epoch=0, vis=False):
        ...
        # Evaluate model
        if args.model_name == "PersFormer" or args.model_name.lower() == "implicitpersformer" :
            model.eval()
        elif args.model_name == "GenLaneNet":
            model2.eval()
        ...
        with torch.no_grad():
            for i, (json_files, input, seg_maps, gt, gt_laneline_img, idx, gt_hcam, gt_pitch, gt_intrinsic, gt_extrinsic, seg_name, seg_bev_map, M, H_g2im, P_g2im, H_crop, H_im2ipm) in tqdm(enumerate(loader)):
                if not args.no_cuda:
                    input, gt = input.cuda(non_blocking=True), gt.cuda(non_blocking=True)
                    seg_maps = seg_maps.cuda(non_blocking=True)
                    gt_hcam = gt_hcam.cuda()
                    ...
                    H_crop   = H_crop.cuda()
                    H_im2ipm = H_im2ipm.cuda()
                assert gt_extrinsic.shape[0]==input.shape[0], "gt_extrinsic: {}, input: {}".format(gt_extrinsic.shape, input.shape)
                assert gt_intrinsic.shape[0]==input.shape[0], "gt_intrinsic: {}, input: {}".format(gt_intrinsic.shape, input.shape)
                ...
                if args.model_name == "PersFormer":
                    ...
                    # add sigmoid to visualize property
                    # output_net[:, :, -3*self.args.num_y_steps:] = torch.sigmoid(output_net[:, :, -3*self.args.num_y_steps:])
                    output_net[:, :, 2*self.args.num_y_steps:3*self.args.num_y_steps] = torch.sigmoid(output_net[:, :, 2*self.args.num_y_steps:3*self.args.num_y_steps])
                    ...
                    # start = time.time()
                    # for _ in range(iterations):
                    #     ane2d_result, lane3d_result_list, pred_hcam, pred_pitch, pred_seg_bev_map, uncertainty_loss = model(input=input, _M_inv=M_inv, aug_mat=None, 
                    #                             H_g2im=H_g2im, P_g2im=P_g2im, H_crop=H_crop, H_im2ipm=H_im2ipm,
                    #                             intrinsic=gt_intrinsic, extrinsic=gt_extrinsic)
                    # torch.cuda.synchronize()
                    # end = time.time()
                    ...
                    lane2d_result, lane3d_result_list, pred_hcam, pred_pitch, pred_seg_bev_map, uncertainty_loss = model(input=input, _M_inv=M_inv, aug_mat=None, 
                                                H_g2im=H_g2im, P_g2im=P_g2im, H_crop=H_crop, H_im2ipm=H_im2ipm,
                                                intrinsic=gt_intrinsic, extrinsic=gt_extrinsic)

                    laneatt_proposals_list = lane2d_result
                    output_net = lane3d_result_list[-1]
                    
                    # loss
                    anchor_y_steps = dataset.anchor_y_steps
                    anchor_x_steps = dataset.anchor_x_steps if dataset.use_default_anchor else dataset.anchor_grid_x   #(N_anchor, N_y)
                    loss = implicit_loss_func( gt_pitch, pred_pitch,  
                                               gt_hcam, pred_hcam,
                                               seg_bev_map, pred_seg_bev_map, 
                                               gt_laneline_img, lane2d_result,  
                    ...
                    # add sigmoid to visualize property
                    # print("-"*30, output_net[:, :, 2*self.args.num_y_steps:3*self.args.num_y_steps].max(), output_net[:, :, 2*self.args.num_y_steps:3*self.args.num_y_steps].min())
                    output_net[:, :, 2*self.args.num_y_steps:3*self.args.num_y_steps] = torch.sigmoid(output_net[:, :, 2*self.args.num_y_steps:3*self.args.num_y_steps])
                    # print("+"*30, output_net.shape, output_net[:, :, 2*self.args.num_y_steps:3*self.args.num_y_steps].max(), output_net[:, :, 2*self.args.num_y_steps:3*self.args.num_y_steps].min())
                ...
                # Visualization
                if ((i + 1) % args.save_freq == 0 or args.evaluate) and \
                   (args.model_name == "PersFormer"  or args.model_name.lower() == "implicitpersformer") :
                    gt_2d = []
                    for j in range(num_el):
                ...
                for j in range(num_el):
                    ...
                    if args.evaluate and (args.model_name == "PersFormer" or args.model_name.lower() == "implicitpersformer") :
                        img_path = json_line["file_path"]
                        if args.dataset_name == 'openlane':
    ...
    def eval(self):
        ...
        if args.model_name.lower() == "persformer" :
            ...
            anchor_y_steps = self.valid_dataset.anchor_y_steps
            anchor_x_steps = self.valid_dataset.anchor_x_steps if self.valid_dataset.use_default_anchor else self.valid_dataset.anchor_grid_x   #(N_anchor, N_y)
            x_off_std      = self.valid_dataset.get_x_off_std()
            z_std          = self.valid_dataset.get_z_std()
            model = ImplicitPersFormer(args, 
                                       anchor_y_steps, anchor_x_steps, 
                                       x_off_std, z_std)
        ...
        # path = 'data_splits/openlane/PersFormerCoarse/model_best_epoch_91.pth.tar'
        ...
        #             if key=="uncertainty_loss" :
        #                 print("- -"*30, type(val), val.shape)
        #                 para = torch.zeros(9)
        #                 para[:7] = val
        #                 state_dict[key] = para
        #             else :
        #                 state_dict[key] = val
        #         elif key.find("lane_out")!=-1 :
        #             key = key.replace("lane_out", "lane3d_head_list.0")
        #             state_dict[key] = val
        #         else :
        #             print(key, val.shape)
        ...
        if os.path.isfile(best_file_name):
            ...
            if (args.proc_id == 0) and (args.model_name == "PersFormer" or args.model_name.lower() == "implicitpersformer"):
    ...
    def visualize_dataset(self):
        ...
        train_loader  = self.train_loader
        # aug_mat
        for i, (json_files, input, seg_maps, gt, gt_laneline_img, idx, gt_hcam, gt_pitch, gt_intrinsic, gt_extrinsic, seg_name, seg_bev_map, raw_image, M) in tqdm(enumerate(train_loader)):
            # idx_json_file: label file;
            # image: (C,H,W), normalized image;
            # seg_label: (H,W), binary mask of lane on image;
            # gt_anchor: 2D: (anchor_num, num_y_steps+num_category), x_off_values with num_y_steps;
            #            3D: (anchor_num, (num_y_steps+num_category)*3)), x_off_values, z_values, visibility with num_y_steps;
            # gt_laneline_img: [[x_2d, y_2d], ...].T, 2D coodinate of lane points on image;
            # idx: int, index;
            # gt_cam_height: float, camera height;
            # gt_cam_pitch: float, camera pitch's degree in PI;
            # intrinsics: (3,3), intrinsic matrix;
            # extrinsics: (3,4), extrinsic matrix;
            # seg_name: string, image file name;
            # seg_bev_map: (H,W), binary mask of lane in BEV;
            # raw_image: (C,H,W), image without normalization;
            from IPython import embed
            embed()
            # Put inputs on gpu if possible
            if not args.no_cuda:
            ...
            vis_img = torch.zeros((B,C,2*H,2*W))
            vis_img[:,:,0:H,0:W] = raw_image
            vis_img[:,:,H:2*H,0:W] = seg_maps*255
            _,_,bev_H,bev_W = seg_bev_map.shape
            vis_img[:,:, (H-bev_H)//2:(H-bev_H)//2+bev_H, W+(W-bev_W)//2:W+(W-bev_W)//2+bev_W] = seg_bev_map*255
            
            if not os.path.exists(os.path.join(self.vis_dir, seg_name[0])) :
                os.mkdir( os.path.join(self.vis_dir, seg_name[0]) )
            
            for batch_idx in range(len(json_files)) :
                time_stamp = json_files[batch_idx].split("/")[-1].split(".")[0]
                path = os.path.join(self.vis_dir, seg_name[batch_idx], "{}.jpg".format(time_stamp))
                save_image(vis_img[batch_idx], path)
                print(path)
                # print("time_stamp: ", time_stamp,
                #       "seg_name: ", seg_name,
                #       "idx: ", idx,
                #       "raw_image: ", raw_image.shape, raw_image.dtype, 
                #       "seg_bev_map: ", seg_bev_map.shape, torch.min(seg_bev_map), torch.max(seg_bev_map),
                #       "seg_maps: ", seg_maps.shape, torch.min(seg_maps), torch.max(seg_maps),
                #       "vis_img: ", vis_img.shape,
                #       sep="   ")
                # break

        generate_video(img_dir=os.path.join(self.vis_dir, seg_name[0]), 
                       sv_dir=os.path.join(self.vis_dir, seg_name[0]),
                       video_name="{}.mp4".format(seg_name[0]))
    ...
    def _get_model_ddp(self, dataset):
        args = self.args
        # Define network
        if args.model_name.lower() == "persformer" :
            model = PersFormer(args)
            ...
            anchor_y_steps = dataset.anchor_y_steps
            anchor_x_steps = dataset.anchor_x_steps if dataset.use_default_anchor else dataset.anchor_grid_x   #(N_anchor, N_y)
            x_off_std      = dataset.get_x_off_std()
            z_std          = dataset.get_z_std()
            model = ImplicitPersFormer(args, 
                                       anchor_y_steps, anchor_x_steps, 
                                       x_off_std, z_std)
        ...
        # path = 'data_splits/openlane/PersFormerCoarse/model_best_epoch_91.pth.tar'
        path = "data_splits/openlane/multiScaleRegCatHie_openlane/model_best_epoch_88.pth.tar"
        if os.path.isfile(path):
            checkpoint = torch.load(path, map_location=torch.device('cpu'))
            model_dict = model.state_dict()
            state_dict = {}
        ...
        if args.model_name == "PersFormer" or args.model_name.lower() == "implicitpersformer":
            if args.freeze :
                ...
                for name, param in model.named_parameters():
                    if name.find("encoder.")>-1 or \
                       name.find("neck.")>-1 or \
                       name.find("shared_encoder.")>-1 or \
                       name.find("laneatt_head.")>-1 or \
                       name.find("pers_tr.")>-1 or \
                       name.find("de_conv.")>-1 or \
                       name.find("bev_head.")>-1 or \
                       name.find("segment_head.")>-1 or \
                       name.find("lane3d_head_list.0.")>-1 :
                        param.requires_grad = False
                        cnt += 1
                print("Freezing {} parameters".format(cnt))
                optimizer = define_optim(args.optimizer, filter(lambda p: p.requires_grad, model.parameters()),
    ...
    def compute_loss(self, args, epoch, loss_3d, loss_att, loss_seg, uncertainty_loss, 
                     loss_3d_dict, loss_att_dict,
                     loss_point3d_occ=None, loss_point3d_vis=None):
        if args.learnable_weight_on:
            ...
            # if args.proc_id == 0 :
            #     print("loss_3d: ", loss.item(), loss_3d_dict['vis_loss'].item(), loss_3d_dict['prob_loss'].item(), loss_3d_dict['reg_loss'].item(),  _3d_vis_loss_factor, _3d_prob_loss_factor, _3d_reg_loss_factor )
            ...
            # if args.proc_id == 0 :
            #     print("loss_3d+loss_2d+seg: ", loss)
            
            if "point_occ" in loss_3d_dict.keys() :
                _3d_point_occ_loss_factor = 1 / torch.exp(uncertainty_loss[7])
                loss += _3d_point_occ_loss_factor * loss_3d_dict['point_occ']
            if "point_vis" in loss_3d_dict.keys() :
                _3d_point_occ_loss_factor = 1 / torch.exp(uncertainty_loss[8])
                loss += _3d_point_occ_loss_factor * loss_3d_dict['point_vis']

        else:
            # epoch depended loss
            if epoch > args.seg_start_epoch:
                loss = loss_3d + args.loss_att_weight * loss_att + args.loss_seg_weight * loss_seg + 0.0 * uncertainty_loss[0:6].sum()
                if loss_point3d_occ is not None :
                    loss += loss_point3d_occ * args.loss_point_weight[0]
                if loss_point3d_vis is not None :
                    loss += loss_point3d_vis * args.loss_point_weight[1]
            else:
